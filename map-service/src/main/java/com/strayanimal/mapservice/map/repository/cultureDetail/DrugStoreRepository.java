...
import com.strayanimal.mapservice.map.entity.CultureDetail.DrugStore;
...
public interface DrugStoreRepository extends JpaRepository<DrugStore, Long> {
    ...
    @Query("SELECT DISTINCT" +
            " p.sigungu" +
            " FROM DrugStore p WHERE p.sido = :sido")
    List<String> findDetailRegion(@Param("sido") String targetRegion);

    // 세종은 sigungu가 없어서, legalDong을 받자.
    @Query("SELECT DISTINCT p.legalDong FROM DrugStore p WHERE p.sido = :sido")
    List<String> findDetailSejong(@Param("sido") String targetRegion);

    @Query("SELECT p FROM DrugStore p WHERE p.sido = :sido AND p.sigungu = :sigungu")
    List<DrugStore> findListByRegion(@Param("sido") String desc, @Param("sigungu") String sigungu);

    // 세종용....
    @Query("SELECT p FROM DrugStore p WHERE p.sido = :sido AND p.legalDong = :legal")
    List<DrugStore> findListByRegionSejong(@Param("sido") String desc, @Param("legal") String legalDong);
}
