...
import com.strayanimal.mapservice.map.entity.CultureDetail.PetCafe;
...
public interface PetCafeRepository extends JpaRepository<PetCafe, Long> {
    ...
    @Query("SELECT DISTINCT " +
            "p.sigungu" +
            " FROM PetCafe p WHERE p.sido = :sido")
    List<String> findDetailRegion(@Param("sido") String targetRegion);

    // 세종은 sigungu가 없어서, legalDong을 받자.
    @Query("SELECT DISTINCT p.legalDong FROM PetCafe p WHERE p.sido = :sido")
    ...
    // 세종용....
    @Query("SELECT p FROM PetCafe p WHERE p.sido = :sido AND p.legalDong = :legal")
    List<PetCafe> findListByRegionSejong(@Param("sido") String desc, @Param("legal") String legalDong);
}
